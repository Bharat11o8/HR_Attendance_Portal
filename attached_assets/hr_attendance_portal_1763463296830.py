...
def process_attendance(records, start_date, end_date):
    ...
    filtered_records = [
        r for r in records
        if r['EnNo'] in EMPLOYEE_MASTER and start_date <= r['Date'] <= end_date
    ]
    ...
    for emp_id in sorted(EMPLOYEE_MASTER.keys()):
        ...
        for date in date_range:
            ...
            if key in daily_attendance:
                ...
                late_threshold = datetime.strptime('09:30:00',
                                                   '%H:%M:%S').time()
                ...
                # Calculate overtime only if out time is after threshold
                if out_time > overtime_threshold:
                    # Convert times to datetime objects for calculation
                    threshold_dt = datetime.combine(date, overtime_threshold)
                    out_dt = datetime.combine(date, out_time)
                    overtime_delta = out_dt - threshold_dt
                    ...
                    overtime_str = f'{hours:02d}:{minutes:02d}:{seconds:02d}'
                else:
                    overtime_str = ''
            else:
                in_str = ''
...
def process():
    try:
        ...
        with pd.ExcelWriter(output,
                            engine='openpyxl') as writer:  # type: ignore
